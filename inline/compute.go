...
import "github.com/jeremiah-masters/dlht/allocator"
...
func (m *Map[V]) LoadOrCompute(key uint64, fn func() (value V, save bool)) (value V, loaded bool) {
...
// called at most once per absent key. Loads remain lock-free.
// Concurrent callers for the same absent key coordinate via a per-key channel.
// If fn returns save=false, the coordination resets and the next caller retries.
func (m *Map[V]) LoadOrComputeOnce(key uint64, fn func() (value V, save bool)) (value V, loaded bool) {
	for {
		if v, ok := m.Get(key); ok {
			// Happy path.
			return v, true
...
func (m *Map[V]) getComputeMap() *allocator.Map[uint64, chan struct{}] {
	if cm := m.compute.Load(); cm != nil {
		return cm
	}
	cm := allocator.New[uint64, chan struct{}](allocator.Options{InitialSize: 16})
